...
import styles from './GoGadgetsStyle'
...
export class GoGadgetsScreen extends Component {
    constructor(props) {
        super(props);
        this.state = {
            restaurantList: [
    ...
    getTrendingFoodListSuccessCallBack = async (res) => {
        console.log('@@@ Get Trending Gadget Data Success CallBack ===================', res);
        ...
        console.log('@@@ Get Trending Gadget Data CallBack ===================', localPlayerList);
        this.setState({ restaurantList: modifiedList, showSearchAFriendsList: true });
    }

    getTrendingFoodListFailureCallBack = (error) => {
        console.log('@@@ Get Trending Gadget Data Failure CallBack ===================', error);
        if (error) {
            setTimeout(() => {
                alert(error);
            }, 0);
    ...
    getAllGadgetCategoryList = async () => {
        this.props.getAllGadgetCategoryList((res) => this.getAllGadgetCategoryListSuccessCallBack(res), (err) => this.getAllGadgetCategoryListFailureCallBack(err))
    }

    getAllGadgetCategoryListSuccessCallBack = async (res) => {
        console.log('@@@ Get All Gadget Category Success CallBack ===================', res);
        let localPlayerList = res.data.data;
        let modifiedList = [];
        localPlayerList.map((item) => {
            item['isSelected'] = false;
    ...
    getAllGadgetCategoryListFailureCallBack = (error) => {
        console.log('@@@ Get All Gadget Category Failure CallBack ===================', error);
        if (error) {
            setTimeout(() => {
                alert(error);
            }, 0);
            ...
            <TouchableOpacity style={styles.restaurantCell} onPress={() => this.props.navigation.navigate("GadgetsDetail", { item: item })}>
                ...
                <View style={{ marginTop: verticalScale(15), marginHorizontal: scale(20) }}>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: verticalScale(5) }}>
                        <Text style={styles.min}>{item.astoreAddressdd}</Text>
                    </View>
                </View>
            </TouchableOpacity>
        );
        ...
        return (
            <View>
                <View style={styles.seeAllView}>
                    <Text style={styles.allRestaurants}>Trending Gadgets</Text>
                    <TouchableOpacity onPress={() => this.props.navigation.navigate("AllGadgets", { restaurantList: this.state.restaurantList })}><Text style={styles.seeAll}>See all ({this.state.restaurantList.length})</Text></TouchableOpacity>
                </View>
                <FlatList
...
const mapDispatchToProps = dispatch => {
    return {
        ...
        getAllGadgetCategoryList: (successCallBack, failureCallBack) => dispatch(homeActions.getAllGadgetCategoryList(successCallBack, failureCallBack)),
    };
};

export default connect(mapStateToProps, mapDispatchToProps)(GoGadgetsScreen)
