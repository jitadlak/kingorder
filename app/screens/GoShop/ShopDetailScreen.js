...
import styles from './ShopDetailStyle'
...
export class ShopDetailScreen extends Component {
    ...
    componentDidMount = () => {
        this.getAllShopMenuList();
    }

    getAllShopMenuList = async () => {
        this.props.getAllShopMenuList((res) => this.getAllShopMenuListSuccessCallBack(res), (err) => this.getAllShopMenuListFailureCallBack(err))
    }

    getAllShopMenuListSuccessCallBack = async (res) => {
        console.log('@@@ Get All Shop Menu Success CallBack ===================', res);
        ...
        console.log('@@@ Get All Shop Menu Data CallBack ===================', localPlayerList);
        this.setState({ allMenuList: modifiedList });
    }

    getAllShopMenuListFailureCallBack = (error) => {
        console.log('@@@ Get All Shop Menu Failure CallBack ===================', error);
        if (error) {
            setTimeout(() => {
                alert(error);
            }, 0);
    ...
    renderMenuCell = (item) => {
        return (
            <TouchableOpacity onPress={() => this.props.navigation.navigate("MenuDetail", {categoryName: "Shop", item: item})}>
        ...
        return (
            <View>
                <View style={styles.seeAllView}>
                    <Text style={styles.allRestaurants}>Products And Photos</Text>
                    <TouchableOpacity
                    onPress={() => this.props.navigation.navigate("ShopMenuList", {itemList: this.state.allMenuList})}
...
const mapDispatchToProps = dispatch => {
    return {
        ...
        getAllShopMenuList: (successCallBack, failureCallBack) => dispatch(homeActions.getAllShopMenuList(successCallBack, failureCallBack)),
    };
};
